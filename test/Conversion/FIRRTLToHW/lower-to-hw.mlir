...
//    stop(clock2, reset, 0)
  ...
  firrtl.module private @SubindexDestination(in %a: !firrtl.uint<1>) {
    %b = firrtl.wire : !firrtl.vector<uint<1>, 3>
    %0 = firrtl.subindex %b[1] : !firrtl.vector<uint<1>, 3>
    firrtl.connect %0, %a : !firrtl.uint<1>, !firrtl.uint<1>
    // CHECK:      [[OUT:%.+]] = hw.wire [[UPDATED:%.+]] : !hw.array<3xi1>
    // CHECK-NEXT: [[SLICE_HI:%.+]] = hw.array_slice %z_i3[%c-2_i2]
    // CHECK-NEXT: [[UPDATED_FIELD:%.+]] = hw.array_create %a
    // CHECK-NEXT: [[SLICE_LO:%.+]] = hw.array_slice %z_i3[%c0_i2]
    // CHECK-NEXT: [[UPDATED]] = hw.array_concat [[SLICE_HI]], [[UPDATED_FIELD]], [[SLICE_LO]]
  }

  // CHECK-LABEL: hw.module private @SubaccessDestination
  firrtl.module private @SubaccessDestination(in %i: !firrtl.uint<2>, in %a: !firrtl.uint<1>) {
    %b = firrtl.wire : !firrtl.vector<uint<1>, 3>
    %0 = firrtl.subaccess %b[%i] : !firrtl.vector<uint<1>, 3>, !firrtl.uint<2>
    firrtl.connect %0, %a : !firrtl.uint<1>, !firrtl.uint<1>
    // CHECK:      [[INIT:%.+]] = sv.constantZ : !hw.array<3xi1>
    // CHECK-NEXT: [[OUT:%.+]] = hw.wire [[UPDATED:%.+]] : !hw.array<3xi1>

    // Index 0
    // CHECK-NEXT: [[EQ_0:%.+]] = comb.icmp eq %i, %c0_i2
    // CHECK-NEXT: [[OLD_0:%.+]] = hw.array_get [[INIT]][%c0_i2]
    // CHECK-NEXT: [[UPDATED_0:%.+]] = comb.mux [[EQ_0]], %a, [[OLD_0]]

    // Index 1
    // CHECK-NEXT: [[EQ_1:%.+]] = comb.icmp eq %i, %c1_i2
    // CHECK-NEXT: [[OLD_1:%.+]] = hw.array_get [[INIT]][%c1_i2]
    // CHECK-NEXT: [[UPDATED_1:%.+]] = comb.mux [[EQ_1]], %a, [[OLD_1]]

    // Index 2
    // CHECK-NEXT: [[EQ_2:%.+]] = comb.icmp eq %i, %c-2_i2
    // CHECK-NEXT: [[OLD_2:%.+]] = hw.array_get [[INIT]][%c-2_i2]
    // CHECK-NEXT: [[UPDATED_2:%.+]] = comb.mux [[EQ_2]], %a, [[OLD_2]]

    // CHECK-NEXT: [[UPDATED]] = hw.array_create [[UPDATED_0]], [[UPDATED_1]], [[UPDATED_2]]
  }

  // CHECK-LABEL: hw.module private @SubfieldDestination
  firrtl.module private @SubfieldDestination(in %a: !firrtl.uint<1>) {
    %b = firrtl.wire : !firrtl.bundle<a: uint<1>, b: uint<1>>
    %0 = firrtl.subfield %b[a] : !firrtl.bundle<a: uint<1>, b: uint<1>>
    firrtl.connect %0, %a : !firrtl.uint<1>, !firrtl.uint<1>
    // CHECK:      [[INIT:%.+]] = sv.constantZ : !hw.struct<a: i1, b: i1>
    // CHECK-NEXT: [[OUT:%.+]] = hw.wire [[UPDATED:%.+]] : !hw.struct<a: i1, b: i1>
    // CHECK-NEXT: [[UPDATED]] = hw.struct_inject [[INIT]]["a"], %a
  }

  // CHECK-LABEL: hw.module private @NestedSubfieldDestination
  firrtl.module private @NestedSubfieldDestination(in %a: !firrtl.uint<1>) {
    %b = firrtl.wire : !firrtl.bundle<a: bundle<b: bundle<c: uint<1>>>>
    %0 = firrtl.subfield %b[a] : !firrtl.bundle<a: bundle<b: bundle<c: uint<1>>>>
    %1 = firrtl.subfield %0[b] : !firrtl.bundle<b: bundle<c: uint<1>>>
    %2 = firrtl.subfield %1[c] : !firrtl.bundle<c: uint<1>>
    firrtl.connect %2, %a : !firrtl.uint<1>, !firrtl.uint<1>
    // CHECK:      [[INIT:%.+]] = sv.constantZ : !hw.struct<a: !hw.struct<b: !hw.struct<c: i1>>>
    // CHECK-NEXT: [[OUT:%.+]] = hw.wire [[UPDATED:%.+]] : !hw.struct<a: !hw.struct<b: !hw.struct<c: i1>>>
    // CHECK-NEXT: [[OLD_A:%.+]] = hw.struct_extract [[INIT]]["a"]
    // CHECK-NEXT: [[OLD_B:%.+]] = hw.struct_extract [[OLD_A]]["b"]
    // CHECK-NEXT: [[UPDATED_B:%.+]] = hw.struct_inject [[OLD_B]]["c"], %a
    // CHECK-NEXT: [[UPDATED_A:%.+]] = hw.struct_inject [[OLD_A]]["b"], [[UPDATED_B]]
    // CHECK-NEXT: [[UPDATED]] = hw.struct_inject [[INIT]]["a"], [[UPDATED_A]]
  }
  ...
  firrtl.module private @BundleConnection(in %a: !firrtl.bundle<d: uint<1>>) {
    %b = firrtl.wire : !firrtl.bundle<c: bundle<d: uint<1>>>
    %0 = firrtl.subfield %b[c] : !firrtl.bundle<c: bundle<d: uint<1>>>
    firrtl.connect %0, %a : !firrtl.bundle<d: uint<1>>, !firrtl.bundle<d: uint<1>>
    // CHECK:      [[INIT:%.+]] = sv.constantZ : !hw.struct<c: !hw.struct<d: i1>>
    // CHECK-NEXT: [[OUT:%.+]] = hw.wire [[UPDATED:%.+]] : !hw.struct<c: !hw.struct<d: i1>>
    // CHECK-NEXT: [[UPDATED]] = hw.struct_inject [[INIT]]["c"], %a
  }
  ...
  // CHECK-LABEL: hw.module @ConnectSubfield(%clock: i1, %value: i2)
  firrtl.module @ConnectSubfield(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.bundle<a: uint<2>>
    %field = firrtl.subfield %reg[a] : !firrtl.bundle<a: uint<2>>
    firrtl.strictconnect %field, %value : !firrtl.uint<2>
    // CHECK-NEXT: %reg = seq.firreg [[TMP:%.+]] clock %clock : !hw.struct<a: i2>
    // CHECK-NEXT: [[TMP]] = hw.struct_inject %reg["a"], %value : !hw.struct<a: i2>
  }

  // CHECK-LABEL: hw.module @ConnectSubfields(%clock: i1, %valueA: i2, %valueB: i3)
  firrtl.module @ConnectSubfields(
    in %clock: !firrtl.clock,
    in %valueA: !firrtl.uint<2>,
    in %valueB: !firrtl.uint<3>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.bundle<a: uint<2>, b: uint<3>>
    %fieldA = firrtl.subfield %reg[a] : !firrtl.bundle<a: uint<2>, b: uint<3>>
    %fieldB = firrtl.subfield %reg[b] : !firrtl.bundle<a: uint<2>, b: uint<3>>
    firrtl.strictconnect %fieldA, %valueA : !firrtl.uint<2>
    firrtl.strictconnect %fieldB, %valueB : !firrtl.uint<3>
    // CHECK-NEXT: %reg = seq.firreg [[AFTER_B:%.+]] clock %clock : !hw.struct<a: i2, b: i3>
    // CHECK-NEXT: [[AFTER_A:%.+]] = hw.struct_inject %reg["a"], %valueA : !hw.struct<a: i2, b: i3>
    // CHECK-NEXT: [[AFTER_B]] = hw.struct_inject [[AFTER_A]]["b"], %valueB : !hw.struct<a: i2, b: i3>
  }

  // CHECK-LABEL: hw.module @ConnectNestedSubfield(%clock: i1, %value: i2)
  firrtl.module @ConnectNestedSubfield(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.bundle<a: bundle<b: uint<2>>>
    %fieldA = firrtl.subfield %reg[a] : !firrtl.bundle<a: bundle<b: uint<2>>>
    %fieldB = firrtl.subfield %fieldA[b] : !firrtl.bundle<b: uint<2>>
    firrtl.strictconnect %fieldB, %value : !firrtl.uint<2>
    // CHECK-NEXT: %reg = seq.firreg [[UPDATED:%.+]] clock %clock : !hw.struct<a: !hw.struct<b: i2>>
    // CHECK-NEXT: [[OLD_A:%.+]] = hw.struct_extract %reg["a"] : !hw.struct<a: !hw.struct<b: i2>>
    // CHECK-NEXT: [[NEW_A:%.+]] = hw.struct_inject [[OLD_A]]["b"], %value : !hw.struct<b: i2>
    // CHECK-NEXT: [[UPDATED]] = hw.struct_inject %reg["a"], [[NEW_A]] : !hw.struct<a: !hw.struct<b: i2>>
  }

  // CHECK-LABEL: hw.module @ConnectNestedFieldsAndIndices(%clock: i1, %value: i2)
  firrtl.module @ConnectNestedFieldsAndIndices(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<bundle<a: vector<bundle<b: uint<2>>, 3>>, 5>
    %field1 = firrtl.subindex %reg[1] : !firrtl.vector<bundle<a: vector<bundle<b: uint<2>>, 3>>, 5>
    ...
    // CHECK-NEXT: hw.constant 2 : i3
    // CHECK-NEXT: hw.constant 0 : i2
    // CHECK-NEXT: hw.constant -2 : i2
    // CHECK-NEXT: hw.constant 1 : i2
    // CHECK-NEXT: hw.constant 1 : i3
    // CHECK-NEXT: %reg = seq.firreg %[[UPDATED:.*]] clock %clock : !hw.array<5xstruct<a: !hw.array<3xstruct<b: i2>>>>
    // CHECK-NEXT: [[OLD_1:%.+]] = hw.array_get %reg[%c1_i3] : !hw.array<5xstruct<a: !hw.array<3xstruct<b: i2>>>>
    // CHECK-NEXT: [[OLD_2:%.+]] = hw.struct_extract [[OLD_1]]["a"] : !hw.struct<a: !hw.array<3xstruct<b: i2>>>
    // CHECK-NEXT: [[OLD_3:%.+]] = hw.array_get [[OLD_2]][%c1_i2] : !hw.array<3xstruct<b: i2>>
    // CHECK-NEXT: [[NEW_3:%.+]] = hw.struct_inject [[OLD_3]]["b"], %value : !hw.struct<b: i2>
    // CHECK-NEXT: [[NEW_2_HI:%.+]] = hw.array_slice [[OLD_2]][%c-2_i2]
    // CHECK-NEXT: [[NEW_2_MID:%.+]] = hw.array_create [[NEW_3]]
    // CHECK-NEXT: [[NEW_2_LO:%.+]] = hw.array_slice [[OLD_2]][%c0_i2]
    // CHECK-NEXT: [[NEW_2:%.+]] = hw.array_concat [[NEW_2_HI]], [[NEW_2_MID]], [[NEW_2_LO]]
    // CHECK-NEXT: [[NEW_1:%.+]] = hw.struct_inject [[OLD_1]]["a"], [[NEW_2]]
    // CHECK-NEXT: [[UPDATED_HI:%.+]] = hw.array_slice %reg[%c2_i3]
    // CHECK-NEXT: [[UPDATED_MID:%.+]] = hw.array_create [[NEW_1]]
    // CHECK-NEXT: [[UPDATED_LO:%.+]] = hw.array_slice %reg[%c0_i3]
    // CHECK-NEXT: [[UPDATED]] = hw.array_concat [[UPDATED_HI]], [[UPDATED_MID]], [[UPDATED_LO]]
  }

  // CHECK-LABEL: hw.module @ConnectSubindex(%clock: i1, %value: i2)
  firrtl.module @ConnectSubindex(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<uint<2>, 3>
    %field = firrtl.subindex %reg[1] : !firrtl.vector<uint<2>, 3>
    firrtl.strictconnect %field, %value : !firrtl.uint<2>
    // CHECK-NEXT: hw.constant 0 : i2
    // CHECK-NEXT: hw.constant -2 : i2
    // CHECK-NEXT: %reg = seq.firreg [[UPDATED:%.+]] clock %clock : !hw.array<3xi2>
    // CHECK-NEXT: [[HI:%.+]] = hw.array_slice %reg[%c-2_i2] : (!hw.array<3xi2>) -> !hw.array<1xi2>
    // CHECK-NEXT: [[MID:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[LO:%.+]] = hw.array_slice %reg[%c0_i2] : (!hw.array<3xi2>) -> !hw.array<1xi2>
    // CHECK-NEXT: [[UPDATED]] = hw.array_concat [[HI]], [[MID]], [[LO]] : !hw.array<1xi2>, !hw.array<1xi2>, !hw.array<1xi2>
  }

  // CHECK-LABEL: hw.module @ConnectSubindexSingleton(%clock: i1, %value: i2)
  firrtl.module @ConnectSubindexSingleton(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<uint<2>, 1>
    %field = firrtl.subindex %reg[0] : !firrtl.vector<uint<2>, 1>
    firrtl.strictconnect %field, %value : !firrtl.uint<2>
    // CHECK-NEXT: %reg = seq.firreg [[INPUT:%.+]] clock %clock : !hw.array<1xi2>
    // CHECK-NEXT: [[INPUT]] = hw.array_create %value : i2
  }

  // CHECK-LABEL: hw.module @ConnectSubindexLHS(%clock: i1, %value: i2)
  firrtl.module @ConnectSubindexLHS(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<uint<2>, 3>
    %field = firrtl.subindex %reg[0] : !firrtl.vector<uint<2>, 3>
    firrtl.strictconnect %field, %value : !firrtl.uint<2>
    // CHECK-NEXT: hw.constant 1 : i2
    // CHECK-NEXT: %reg = seq.firreg [[INPUT:%.+]] clock %clock : !hw.array<3xi2>
    // CHECK-NEXT: [[REST:%.+]] = hw.array_slice %reg[%c1_i2] : (!hw.array<3xi2>) -> !hw.array<2xi2>
    // CHECK-NEXT: [[ELEM:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[INPUT]] = hw.array_concat [[REST]], [[ELEM]] : !hw.array<2xi2>, !hw.array<1xi2>
  }

  // CHECK-LABEL: hw.module @ConnectSubindexRHS(%clock: i1, %value: i2)
  firrtl.module @ConnectSubindexRHS(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<uint<2>, 3>
    %field = firrtl.subindex %reg[2] : !firrtl.vector<uint<2>, 3>
    firrtl.strictconnect %field, %value : !firrtl.uint<2>
    // CHECK-NEXT: hw.constant 0 : i2
    // CHECK-NEXT: %reg = seq.firreg [[INPUT:%.+]] clock %clock : !hw.array<3xi2>
    // CHECK-NEXT: [[ELEM:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[REST:%.+]] = hw.array_slice %reg[%c0_i2] : (!hw.array<3xi2>) -> !hw.array<2xi2>
    // CHECK-NEXT: [[INPUT]] = hw.array_concat  [[ELEM]], [[REST]] : !hw.array<1xi2>, !hw.array<2xi2>
  }

  // CHECK-LABEL: hw.module @ConnectSubindices(%clock: i1, %value: i2)
  firrtl.module @ConnectSubindices(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<uint<2>, 5>
    %field1 = firrtl.subindex %reg[1] : !firrtl.vector<uint<2>, 5>
    %field2 = firrtl.subindex %reg[2] : !firrtl.vector<uint<2>, 5>
    %field5 = firrtl.subindex %reg[4] : !firrtl.vector<uint<2>, 5>
    ...
    // CHECK-NEXT: hw.constant 3 : i3
    // CHECK-NEXT: hw.constant 0 : i3
    // CHECK-NEXT: hw.constant 2 : i3
    // CHECK-NEXT: %reg = seq.firreg [[UPDATED:%.+]] clock %clock : !hw.array<5xi2>
    // CHECK-NEXT: [[L0_HI:%.+]] = hw.array_slice %reg[%c2_i3] : (!hw.array<5xi2>) -> !hw.array<3xi2>
    // CHECK-NEXT: [[L0_MID:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[L0_LO:%.+]] = hw.array_slice %reg[%c0_i3] : (!hw.array<5xi2>) -> !hw.array<1xi2>
    // CHECK-NEXT: [[L0:%.+]] = hw.array_concat [[L0_HI]], [[L0_MID]], [[L0_LO]] : !hw.array<3xi2>, !hw.array<1xi2>, !hw.array<1xi2>
    // CHECK-NEXT: [[L1_HI:%.+]] = hw.array_slice [[L0]][%c3_i3] : (!hw.array<5xi2>) -> !hw.array<2xi2>
    // CHECK-NEXT: [[L1_MID:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[L1_LO:%.+]] = hw.array_slice [[L0]][%c0_i3] : (!hw.array<5xi2>) -> !hw.array<2xi2>
    // CHECK-NEXT: [[L1:%.+]] = hw.array_concat [[L1_HI]], [[L1_MID]], [[L1_LO]] : !hw.array<2xi2>, !hw.array<1xi2>, !hw.array<2xi2>
    // CHECK-NEXT: [[L3_MID:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[L3_LO:%.+]] = hw.array_slice [[L1]][%c0_i3] : (!hw.array<5xi2>) -> !hw.array<4xi2>
    // CHECK-NEXT: [[UPDATED]] = hw.array_concat [[L3_MID]], [[L3_LO]] : !hw.array<1xi2>, !hw.array<4xi2>
  }

  // CHECK-LABEL: hw.module @ConnectNestedSubindex(%clock: i1, %value: i2)
  firrtl.module @ConnectNestedSubindex(
    in %clock: !firrtl.clock,
    in %value: !firrtl.uint<2>
  ) {
    %reg = firrtl.reg %clock: !firrtl.clock, !firrtl.vector<vector<uint<2>, 3>, 3>
    %field0 = firrtl.subindex %reg[1] : !firrtl.vector<vector<uint<2>, 3>, 3>
    %field1 = firrtl.subindex %field0[1] : !firrtl.vector<uint<2>, 3>
    firrtl.strictconnect %field1, %value : !firrtl.uint<2>
    // CHECK-NEXT: hw.constant 0 : i2
    // CHECK-NEXT: hw.constant -2 : i2
    // CHECK-NEXT: hw.constant 1 : i2
    // CHECK-NEXT: %reg = seq.firreg [[UPDATED:%.+]] clock %clock : !hw.array<3xarray<3xi2>>
    // CHECK-NEXT: [[OLD_0:%.+]] = hw.array_get %reg[%c1_i2] : !hw.array<3xarray<3xi2>>
    // CHECK-NEXT: [[NEW_1_HI:%.+]] = hw.array_slice [[OLD_0]][%c-2_i2] : (!hw.array<3xi2>) -> !hw.array<1xi2>
    // CHECK-NEXT: [[NEW_1_MID:%.+]] = hw.array_create %value : i2
    // CHECK-NEXT: [[NEW_1_LO:%.+]] = hw.array_slice [[OLD_0]][%c0_i2] : (!hw.array<3xi2>) -> !hw.array<1xi2>
    // CHECK-NEXT: [[NEW_1:%.+]] = hw.array_concat [[NEW_1_HI]], [[NEW_1_MID]], [[NEW_1_LO]] : !hw.array<1xi2>, !hw.array<1xi2>, !hw.array<1xi2>
    // CHECK-NEXT: [[UPDATED_HI:%.+]] = hw.array_slice %reg[%c-2_i2] : (!hw.array<3xarray<3xi2>>) -> !hw.array<1xarray<3xi2>>
    // CHECK-NEXT: [[UPDATED_MID:%.+]] = hw.array_create [[NEW_1]] : !hw.array<3xi2>
    // CHECK-NEXT: [[UPDATED_LO:%.+]] = hw.array_slice %reg[%c0_i2] : (!hw.array<3xarray<3xi2>>) -> !hw.array<1xarray<3xi2>>
    // CHECK-NEXT: [[UPDATED]] = hw.array_concat [[UPDATED_HI]], [[UPDATED_MID]], [[UPDATED_LO]] : !hw.array<1xarray<3xi2>>, !hw.array<1xarray<3xi2>>, !hw.array<1xarray<3xi2>>
  }
  ...
  firrtl.module @LowerReadArrayInoutIntoArrayGet(in %a: !firrtl.uint<10>, out %b: !firrtl.uint<10>) {
    ...
    // CHECK:      %r = hw.wire [[UPDATED:%.+]] : !hw.array<1xi10>
    // CHECK-NEXT: [[RET:%.+]] = hw.array_get %r[%false] : !hw.array<1xi10>, i1
    // CHECK-NEXT: [[UPDATED]] = hw.array_create %a
