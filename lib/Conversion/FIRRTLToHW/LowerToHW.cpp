...
/// Resolve a connection to an `hw::WireOp` or `seq::FirRegOp` by computing the
/// overall updated value, resolving through subaccesses, and then updating the
/// input operand to the wire or register to this value.
///
/// The `dest` value is expected to be the unlowered FIRRTL value, such that we
/// can properly trace through all the subaccess operations. The `srcVal` is
/// expected to be the lowered non-FIRRTL value.
FailureOr<bool> FIRRTLLowering::lowerConnect(Value dest, Value srcVal) {
  // This connect lowering is implemented as follows:
  //
  // - First, `lowerConnect` walks the unlowered LHS backwards through any
  //   subindex, subfield, and subaccess ops, to arrive at the root FIRRTL wire
  //   or reg being updated.
  //
  // - We then get the lowered value of that wire or reg, which is going to be a
  //   `seq::FirRegOp` or `hw::WireOp`, and take its current "next" or "input"
  //   operand. This is essentially the currently assigned value, reflecting all
  //   the connects we have seen so far.
  //
  // - We then create new lowerings for any subaccesses on the LHS, on this
  //   currently-assigned value. This is necessary since the lowered subaccesses
  //   that were already created earlier access the lowered *wire* or *reg*, and
  //   not the *current value* assigned to the wire or reg.
  //
  // - Once we have the current value of the subfield we are assigning, we take
  //   the lowered RHS of the connect and walk the subaccesses backwards,
  //   splicing the updated value into the subaccessed aggregates, until we
  //   finally arrive at the root again and have a new RHS for the entire wire
  //   or reg with the connected value spliced in at the right location.
  //
  // - Finally we update the root `seq::FirRegOp` or `hw::WireOp` such that its
  //   "next" or "input" operand corresponds to this new RHS value.

  LLVM_DEBUG({
    llvm::dbgs() << "Lowering connect\n";
    llvm::dbgs() << "  LHS: " << dest << "\n";
    llvm::dbgs() << "  RHS: " << srcVal << "\n";
  });

  // The lowered destination may become unused if it is a subaccess that is only
  // used in this connect.
  if (auto destVal = getPossiblyInoutLoweredValue(dest))
    maybeUnused(destVal);

  // Look through subaccess ops and collect them into a list until we find the
  // "root" value being connected to.
  SmallVector<std::pair<Operation *, Value>> subaccesses;
  while (isa_and_nonnull<SubindexOp, SubaccessOp, SubfieldOp>(
      dest.getDefiningOp())) {
    auto nextDest = dest.getDefiningOp()->getOperand(0);
    if (!getPossiblyInoutLoweredValue(nextDest)) {
      LLVM_DEBUG(llvm::dbgs()
                 << "    Aborting at unlowered " << nextDest << "\n");
      break;
    }
    subaccesses.emplace_back(dest.getDefiningOp(), Value{});
    LLVM_DEBUG(llvm::dbgs() << "    Subaccess: " << dest << "\n");
    dest = nextDest;
  }

  // Find the value currently assigned to the wire or register that we are
  // connecting to. We want to take this value, update it with the connected
  // value, and then assign it to the wire or register. Bail if the root value
  // we're assigning is not a wire or register for which we know how to update
  // the input operand.
  auto destVal = getPossiblyInoutLoweredValue(dest);
  LLVM_DEBUG(llvm::dbgs() << "  Lowered LHS root: " << destVal << "\n");
  auto *destOp = destVal.getDefiningOp();
  if (!destOp)
    return false;
  auto currentVal =
      TypeSwitch<Operation *, Value>(destOp)
          .Case<hw::WireOp>([](auto op) { return op.getInput(); })
          .Case<seq::FirRegOp>([](auto op) { return op.getNext(); })
          .Default([](auto) { return Value{}; });
  if (!currentVal)
    return false;
  LLVM_DEBUG(llvm::dbgs() << "  Current value: " << currentVal << "\n");

  // The `subaccesses` operate on the final value of the wire or register.
  // In order to materialize the connection, we need to take the current
  // value assigned to the wire or register *at the point of the connect*
  // instead. We can then use this current value and inject the connected
  // new value, and then assign the resulting new value as the wire's or
  // register's current value after the connect. To do this, we need to
  // create subaccess operations that mirror `subaccesses` but which operate
  // on that current value.
  for (auto &[subaccessOp, subaccessInput] : llvm::reverse(subaccesses)) {
    FailureOr<Value> result =
        TypeSwitch<Operation *, FailureOr<Value>>(subaccessOp)
            .Case<SubindexOp>(
                [&](auto op) { return lowerSubindex(op, currentVal); })
            .Case<SubaccessOp>(
                [&](auto op) { return lowerSubaccess(op, currentVal); })
            .Case<SubfieldOp>(
                [&](auto op) { return lowerSubfield(op, currentVal); });
    if (failed(result))
      return failure();
    subaccessInput = currentVal;
    currentVal = *result;
    subaccessOp = currentVal.getDefiningOp();
    maybeUnused(currentVal); // new subaccess may be ignored if connect replaces
                             // entire value
    LLVM_DEBUG(llvm::dbgs() << "    Subaccess: " << currentVal << "\n");
  }

  // Iterate through the subaccesses, starting from the innermost one, and
  // insert `srcVal` into the array or struct that was subaccessed. This takes
  // the value to be updated, inserts it into the surrounding aggregate, then
  // takes that new aggregate value and inserts it into its own surrounding
  // aggregate, all the way up to the wire or register.
  for (auto [subaccessOp, subaccessInput] : subaccesses) {
    // Handle connections to struct fields. All that's needed is to take the
    // connected `srcVal` and inject it into the struct itself.
    if (auto op = dyn_cast<hw::StructExtractOp>(subaccessOp)) {
      srcVal = builder.create<hw::StructInjectOp>(subaccessInput,
                                                  op.getFieldAttr(), srcVal);
      LLVM_DEBUG(llvm::dbgs() << "  Injected field " << op.getFieldAttr()
                              << ": " << srcVal << "\n");
      continue;
    }

    // The remainder deals with connections to array elements.
    // TODO: Switch this to an `hw::ArrayInjectOp` once it exists.
    auto op = cast<hw::ArrayGetOp>(subaccessOp);
    auto ty = hw::type_cast<hw::ArrayType>(subaccessInput.getType());
    auto elemTy = ty.getElementType();
    auto size = ty.getSize();
    auto indexWidth = getBitWidthFromVectorSize(size);

    // Handle the case where the index is a constant. In that case we can build
    // a new array by concatenating predecessors, the new value and any
    // successors from the old array.
    if (auto constOp = op.getIndex().template getDefiningOp<hw::ConstantOp>()) {
      // Slice around the updated element and concatenate the
      // results. Higher indices are passed to the concat op first.
      auto index = constOp.getValue().getLimitedValue();
      SmallVector<Value> elements;
      if (index + 1 != size) {
        elements.push_back(builder.create<hw::ArraySliceOp>(
            hw::ArrayType::get(elemTy, size - index - 1), subaccessInput,
            getOrCreateIntConstant(indexWidth, index + 1)));
      }
      elements.push_back(builder.create<hw::ArrayCreateOp>(srcVal));
      if (index != 0) {
        elements.push_back(builder.create<hw::ArraySliceOp>(
            hw::ArrayType::get(elemTy, index), subaccessInput,
            getOrCreateIntConstant(indexWidth, 0)));
      }
      if (elements.size() == 1)
        srcVal = elements.back();
      else
        srcVal = builder.create<hw::ArrayConcatOp>(elements);
      LLVM_DEBUG(llvm::dbgs()
                 << "  Injected index " << index << ": " << srcVal << "\n");
      continue;
    }

    // Handle the case where the index is dynamic. In that case we need to
    // create a mux for each element that updates the element value if the
    // accessed index matches.
    SmallVector<Value> elements;
    for (unsigned idx = 0, end = ty.getSize(); idx != end; ++idx) {
      auto idxVal = getOrCreateIntConstant(indexWidth, idx);
      auto cmpOp = builder.create<comb::ICmpOp>(
          comb::ICmpPredicate::eq, op.getIndex(), idxVal, /*twoState=*/true);
      auto oldVal = builder.create<hw::ArrayGetOp>(subaccessInput, idxVal);
      auto muxOp =
          builder.create<comb::MuxOp>(cmpOp, srcVal, oldVal, /*twoState=*/true);
      elements.push_back(muxOp);
    }
    srcVal = builder.create<hw::ArrayCreateOp>(elements);
    LLVM_DEBUG(llvm::dbgs() << "  Injected dynamic index: " << srcVal << "\n");
  }

  // Assign the updated value `srcVal` as the current value of the wire or
  // register.
  LLVM_DEBUG(llvm::dbgs() << "  Final value: " << srcVal << "\n");
  TypeSwitch<Operation *>(destVal.getDefiningOp())
      .Case<hw::WireOp>([&](auto op) {
        maybeUnused(op.getInput());
        return op.getInputMutable().assign(srcVal);
      })
      .Case<seq::FirRegOp>([&](auto op) {
        maybeUnused(op.getNext());
        return op.getNextMutable().assign(srcVal);
      });
  LLVM_DEBUG(llvm::dbgs() << "  Updated: " << destVal << "\n");
  return true;
}
...
LogicalResult FIRRTLLowering::visitStmt(ConnectOp op) {
  ...
  auto result = lowerConnect(dest, srcVal);
  if (failed(result))
    return failure();
  if (*result)
    return success();
