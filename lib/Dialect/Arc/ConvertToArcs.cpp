...
struct ConvertToArcsPass : public ConvertToArcsBase<ConvertToArcsPass> {
  ...
  /// The set of arc-breaking ops an operation in the current module contributes
  /// to, represented as a bit mask.
  DenseMap<Operation *, APInt> faninMasks;
  /// The sets of operations that contribute to the same arc-breaking ops.
  DenseMap<APInt, DenseSet<Operation *>> faninMaskGroups;
...
LogicalResult ConvertToArcsPass::runOnModule(HWModuleOp module) {
  ...
  if (module.getBodyBlock()->begin() == module.getBodyBlock()->end() &&
  ...
  // For each operation, figure out the set of breaker ops it contributes to, in
  // the form of a bit mask. Then group operations together that contribute to
  // the same set of breaker ops.
  if (failed(analyzeFanIn()))
    return failure();

  // Extract the fanin mask groups into separate combinational arcs and combine
  // them with the registers in the design.
  extractArcs(module);
  absorbRegs(module);
  return success();
}

LogicalResult ConvertToArcsPass::analyzeFanIn() {
  ...
  // Compute fanin masks in reverse post-order, which will compute the mask for
  // an operation's uses before it computes it for the operation itself. This
  // allows us to compute the set of arc breakers an operation contributes to in
  // one pass.
...
void ConvertToArcsPass::extractArcs(HWModuleOp module) {
  ...
  for (auto &group : faninMaskGroups) {
    ...
    //            << "- Extracting arc of " << opSet.size() << " ops\n");
    ...
    for (auto *op : postOrder) {
      if (!opSet.contains(op))
        continue;
      lastOp = op;
      // LLVM_DEBUG(llvm::dbgs() << "  - Extracting " << *op << "\n");
      op->remove();
      builder.insert(op);
      ...
      for (auto result : op->getResults()) {
        ...
        if (anyExternal) {
          // LLVM_DEBUG(llvm::dbgs() << "    - Adding " << result.getType()
          //                         << " output for result "
    ...
    // Create the call to the arc definition to replace the operations that we
    // have just extracted.
...
void ConvertToArcsPass::absorbRegs(HWModuleOp module) {
  // Handle the trivial cases where all of an arc's results are used by exactly
  // one register each.
  unsigned outIdx = 0;
  unsigned numTrivialRegs = 0;
  for (unsigned inIdx = 0, endIdx = arcUses.size(); inIdx != endIdx; ++inIdx) {
    auto &arc = arcUses[inIdx];
    Value clock = arc.clock();
    ...
    for (auto result : arc.getResults()) {
      ...
      auto regOp = dyn_cast<seq::CompRegOp>(use.getOwner());
      if (!regOp || regOp.input() != result ||
          (clock && clock != regOp.clk())) {
        isTrivial = false;
        break;
      }
      clock = regOp.clk();
      absorbedRegs.push_back(regOp);
      absorbedNames[result.getResultNumber()] = regOp.nameAttr();
    }
    if (!isTrivial) {
      arcUses[outIdx++] = arc;
      continue;
    ...
    auto newArc = builder.create<StateOp>(arc.getLoc(), arc.arcAttr(),
                                          arc.getResultTypes(), clock, Value{},
                                          arc.latency() + 1, arc.getOperands());
    newArc->setAttr("names", builder.getArrayAttr(absorbedNames));
    for (auto [arcResult, reg] : llvm::zip(newArc.getResults(), absorbedRegs)) {
  ...
  // Group the remaining registers by the operation they use as input. This will
  // allow us to generally collapse registers derived from the same arc into one
  // shuffling arc.
  DenseMap<std::pair<Value, Operation *>, SmallVector<seq::CompRegOp>>
      regsByInput;
  for (auto *op : arcBreakers)
    if (auto regOp = dyn_cast_or_null<seq::CompRegOp>(op))
      regsByInput[{regOp.clk(), regOp.input().getDefiningOp()}].push_back(
          regOp);
  ...
  for (auto [clockAndOp, regOps] : regsByInput) {
    ...
    for (auto regOp : regOps) {
      auto it = mapping.find(regOp.input());
      if (it == mapping.end()) {
        it = mapping.insert({regOp.input(), inputs.size()}).first;
        inputs.push_back(regOp.input());
...
namespace circt {
namespace arc {
std::unique_ptr<Pass> createConvertToArcsPass() {
