...
struct Emitter {
  Emitter(llvm::raw_ostream &os, const FIREmitterOptions &options)
  ...
  void emitBinaryExpression(Operation *op, StringRef mnemonic);
  void emitUnaryExpression(Operation *op, StringRef mnemonic);
  void emitExpression(BitsPrimOp op);
  void emitExpression(HeadPrimOp op);
  void emitExpression(TailPrimOp op);
  void emitExpression(PadPrimOp op);
  void emitExpression(MuxPrimOp op);
  void emitExpression(ShlPrimOp op);
  void emitExpression(ShrPrimOp op);

  // Funnel all binary ops into `emitBinaryExpression`.
#define HANDLE(OPTYPE, MNEMONIC)                                               \
  void emitExpression(OPTYPE op) { emitBinaryExpression(op, MNEMONIC); }
...
#undef HANDLE
  ...
  // Funnel all unary ops into `emitUnaryExpression`.
#define HANDLE(OPTYPE, MNEMONIC)                                               \
  void emitExpression(OPTYPE op) { emitUnaryExpression(op, MNEMONIC); }
...
private:
  ...
  Optional<StringRef> lookupEmittedName(Value value) {
    auto it = emittedNames.find(value);
    if (it != emittedNames.end())
      return {it->second};
    return {};
  }
...
private:
  ...
  /// reference it.
  DenseMap<Value, SmallString<16>> emittedNames;
};
} // namespace

LogicalResult Emitter::finalize() { return failure(encounteredError); }

/// Emit a string in double quotes, with appropriate escaping, to `os`.
static void emitString(raw_ostream &os, StringRef str) {
  os << "\"";
  for (char c : str) {
    switch (c) {
    case '"':
    ...
    for (auto param : *params) {
      ...
      TypeSwitch<Attribute>(param.second)
          ...
          .Case<FloatAttr>([&](auto attr) {
            SmallString<16> str;
            // CAVEAT: The precision of 8 digits is totally arbitrary and as all
            // things float is bound to come back and bite us.
            attr.getValue().toString(str, /*FormatPrecision=*/8);
            os << str;
          })
          .Case<StringAttr>([&](auto attr) { emitString(os, attr.getValue()); })
          .Default([&](auto attr) {
...
void Emitter::emitModulePorts(ArrayRef<ModulePortInfo> ports,
  ...
  for (unsigned i = 0, e = ports.size(); i < e; ++i) {
    ...
    if (!arguments.empty())
      emittedNames.insert({arguments[i], name});
    os << name << " : ";
    emitType(port.type);
    os << "\n";
  }
...
void Emitter::emitStatementsInBlock(Block &block) {
  for (auto &bodyOp : block) {
    // TODO: Add option to cause non-trivial expressions (everything besides
    // constants and subfield/subindex/subaccess ops) to be emitted out-of-line
...
void Emitter::emitStatement(WireOp op) {
  emittedNames.insert({op, op.name()});
  indent() << "wire " << op.name() << " : ";
  emitType(op.getType());
  emitLocationAndNewLine(op);
}
...
void Emitter::emitStatement(InstanceOp op) {
  ...
  auto modulePorts = getModulePortInfo(op.getReferencedModule());
  for (unsigned i = 0, e = op.getNumResults(); i < e; ++i) {
    portName.append(modulePorts[i].name.getValue());
    emittedNames.insert({op.getResult(i), portName});
...
void Emitter::emitBinaryExpression(Operation *op, StringRef mnemonic) {
  os << mnemonic << "(";
...
void Emitter::emitUnaryExpression(Operation *op, StringRef mnemonic) {
  os << mnemonic << "(";
  emitExpression(op->getOperand(0));
  os << ")";
}

void Emitter::emitExpression(BitsPrimOp op) {
  os << "bits(";
  emitExpression(op.input());
  os << ", " << op.hi() << ", " << op.lo() << ")";
}

void Emitter::emitExpression(HeadPrimOp op) {
  os << "head(";
  emitExpression(op.input());
  os << ", " << op.amount() << ")";
}

void Emitter::emitExpression(TailPrimOp op) {
  os << "head(";
  emitExpression(op.input());
  os << ", " << op.amount() << ")";
}

void Emitter::emitExpression(PadPrimOp op) {
  os << "pad(";
  emitExpression(op.input());
  os << ", " << op.amount() << ")";
}

void Emitter::emitExpression(MuxPrimOp op) {
  os << "mux(";
  emitExpression(op.sel());
  os << ", ";
  emitExpression(op.high());
  os << ", ";
  emitExpression(op.low());
  os << ")";
}

void Emitter::emitExpression(ShlPrimOp op) {
  os << "shl(";
  emitExpression(op.input());
  os << ", " << op.amount() << ")";
}

void Emitter::emitExpression(ShrPrimOp op) {
  os << "shr(";
  emitExpression(op.input());
  os << ", " << op.amount() << ")";
}
...
mlir::LogicalResult
circt::firrtl::exportFIRRTL(mlir::ModuleOp module, llvm::raw_ostream &os,
                            const FIREmitterOptions &options) {
  Emitter emitter(os, options);
...
void circt::firrtl::registerToFIRRTLTranslation() {
  static mlir::TranslateFromMLIRRegistration toFIR(
      "export-firrtl",
      [](ModuleOp module, llvm::raw_ostream &os) {
        return exportFIRRTL(module, os);
      },
